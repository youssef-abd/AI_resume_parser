...
# --- API-prefixed mirror routes for reverse proxy compatibility ---
@app.get("/api/readyz")
def readyz_api():
    return readyz()


@app.post("/api/upload_resume", response_model=UploadResumeResponse, tags=["resumes"])  # type: ignore[arg-type]
async def upload_resume_api(
    file: UploadFile = File(..., description="Resume file: PDF or DOCX"),
    candidate_name: Optional[str] = Form(default=None),
):
    return await upload_resume(file=file, candidate_name=candidate_name)


@app.post("/api/upload_resumes", response_model=List[UploadResumeResponse], tags=["resumes"])  # type: ignore[arg-type]
async def upload_resumes_api(
    files: List[UploadFile] = File(..., description="One or more resume files: PDF or DOCX"),
    candidate_names: Optional[List[str]] = Form(default=None, description="Optional names aligned by index"),
):
    return await upload_resumes(files=files, candidate_names=candidate_names)


@app.post("/api/upload_job", response_model=UploadJobResponse, tags=["jobs"])  # type: ignore[arg-type]
async def upload_job_api(payload: JobIn):
    return await upload_job(payload)


@app.post("/api/upload_job_form", response_model=UploadJobResponse, tags=["jobs"])  # type: ignore[arg-type]
async def upload_job_form_api(
    title: str = Form(...),
    description: str = Form(...),
    required_skills: Optional[str] = Form(default=None, description="Comma or newline separated list"),
):
    return await upload_job_form(title=title, description=description, required_skills=required_skills)


@app.get("/api/match", response_model=MatchResponse, tags=["matching"])  # type: ignore[arg-type]
async def match_api(job_id: str, k: int = 20):
    return await match(job_id=job_id, k=k)


@app.get("/api/resumes/{resume_id}", response_model=ResumeOut, tags=["resumes"])  # type: ignore[arg-type]
async def get_resume_text_api(resume_id: str):
    return await get_resume_text(resume_id)


@app.get("/api/resumes", response_model=List[ResumeListItem], tags=["resumes"])  # type: ignore[arg-type]
async def list_resumes_api(limit: int = 50, offset: int = 0):
    return await list_resumes(limit=limit, offset=offset)


@app.delete("/api/resume/{resume_id}", tags=["resumes"])  # type: ignore[arg-type]
async def delete_resume_api(resume_id: str):
    return await delete_resume(resume_id)


@app.get("/api/jobs/{job_id}", response_model=JobOut, tags=["jobs"])  # type: ignore[arg-type]
async def get_job_info_api(job_id: str):
    return await get_job_info(job_id)


@app.get("/api/jobs", response_model=List[JobListItem], tags=["jobs"])  # type: ignore[arg-type]
async def list_jobs_api(limit: int = 50, offset: int = 0):
    return await list_jobs(limit=limit, offset=offset)


@app.delete("/api/job/{job_id}", tags=["jobs"])  # type: ignore[arg-type]
